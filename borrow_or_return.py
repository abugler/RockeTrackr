...
def borrow_return(TrackingSheet, InventorySheet, NewRowIndex):
    ...
    ChangedItemCell = sheet_helpers.find_item_from_sku(InventorySheet, int(NewRow[2].value))
    ...
    if str(NewRow[3].value) == 'Returning':
        ...
        EndValue = int(InventorySheet.cell(ChangedItemCell.row, 4).value) + int(NewRow[4].value)
        # Find the quantity of that given item that exists in total (currently in stock and being borrowed).
        MaxValue = int(InventorySheet.cell(ChangedItemCell.row, 3).value)
    ...
    # Check if the intention of the user is to borrow an item.
        ...
        EndValue = int(InventorySheet.cell(ChangedItemCell.row, 4).value) - int(NewRow[5].value)
